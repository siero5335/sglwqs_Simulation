...
truth_support_tolerance <- function() {
  if (exists("r2r2_truth_tolerance", mode = "function")) {
    return(r2r2_truth_tolerance())
  }
  1e-12
}

truth_atomic_key_columns <- function(df, include_group = FALSE) {
  keys <- c(
    "battery", "scenario_id", "family", "method", "data_seed",
    "split_replicate", "n", "p"
  )
  if (isTRUE(include_group)) keys <- c(keys, "group")
  intersect(keys, names(df))
}

truth_compound_key <- function(df, keys) {
  values <- lapply(df[keys], function(x) {
    x <- as.character(x)
    x[is.na(x)] <- "<NA>"
    x
  })
...
truth_row_match <- function(df, row, keys) {
  keep <- rep(TRUE, nrow(df))
  for (key in keys) {
    value <- row[[key]][[1L]]
    keep <- keep & if (is.na(value)) is.na(df[[key]]) else !is.na(df[[key]]) & df[[key]] == value
  }
  keep
}

truth_support_correction_audit <- function(component_metrics,
                                           tolerance = truth_support_tolerance()) {
  if (!is.data.frame(component_metrics) || !nrow(component_metrics)) return(data.frame())
  new_active <- abs(component_metrics$true_beta) > tolerance
  new_direction <- ifelse(component_metrics$true_beta > tolerance, "Positive",
    ifelse(component_metrics$true_beta < -tolerance, "Negative", "None")
  )
  changed <- is.na(component_metrics$is_active) |
    component_metrics$is_active != new_active |
    is.na(component_metrics$true_direction) |
    component_metrics$true_direction != new_direction
  if (!any(changed)) return(data.frame())
  audit_columns <- intersect(c(
    "battery", "scenario_id", "family", "method", "data_seed",
    "split_replicate", "n", "p", "variable", "group", "true_beta"
  ), names(component_metrics))
  out <- component_metrics[changed, audit_columns, drop = FALSE]
  out$old_is_active <- component_metrics$is_active[changed]
  out$new_is_active <- new_active[changed]
  out$old_true_direction <- component_metrics$true_direction[changed]
  out$new_true_direction <- new_direction[changed]
  out$truth_tolerance <- tolerance
  rownames(out) <- NULL
  out
}

correct_component_truth_support <- function(component_metrics,
                                            tolerance = truth_support_tolerance()) {
  if (!is.data.frame(component_metrics) || !nrow(component_metrics)) return(component_metrics)
  component_metrics$is_active <- abs(component_metrics$true_beta) > tolerance
  component_metrics$true_direction <- ifelse(component_metrics$true_beta > tolerance, "Positive",
    ifelse(component_metrics$true_beta < -tolerance, "Negative", "None")
  )
  component_metrics$direction_correct <- component_metrics$is_active &
    component_metrics$estimated_direction == component_metrics$true_direction
  component_metrics$sign_assignment_correct <- ifelse(
    component_metrics$is_active,
    component_metrics$estimated_direction == component_metrics$true_direction,
    component_metrics$estimated_direction == "None"
  )
  newly_active_tier <- component_metrics$is_active &
    (is.na(component_metrics$effect_tier) | component_metrics$effect_tier == "null")
  component_metrics$effect_tier[newly_active_tier] <- "active"

  group_keys <- truth_atomic_key_columns(component_metrics, include_group = TRUE)
  group_ids <- truth_compound_key(component_metrics, group_keys)
  for (indices in split(seq_len(nrow(component_metrics)), group_ids)) {
    directions <- unique(component_metrics$true_direction[indices])
    active_directions <- directions[directions != "None" & !is.na(directions)]
    component_metrics$is_active_group[indices] <- any(component_metrics$is_active[indices])
    component_metrics$group_type[indices] <- if (!length(active_directions)) {
      "null"
    } else if (length(active_directions) == 1L) {
      "unidirectional"
    } else {
      "mixed-direction"
    }
...
component_metric_summary_base <- function(component_metrics) {
  finite_mean <- function(x) {
    x <- x[!is.na(x)]
    if (length(x)) mean(x) else NA_real_
  }
  finite_sum <- function(x) {
    if (any(is.finite(x))) sum(x, na.rm = TRUE) else NA_real_
  }
  active <- component_metrics$is_active %in% TRUE
  data.frame(
    active_direction_accuracy = finite_mean(component_metrics$direction_correct[active]),
    sign_assignment_accuracy = finite_mean(component_metrics$sign_assignment_correct),
    active_attribution = finite_sum(component_metrics$normalized_attribution[active]),
    null_attribution = finite_sum(component_metrics$normalized_attribution[!active]),
    active_selection_frequency = finite_mean(component_metrics$selection_frequency[active]),
    null_selection_frequency = finite_mean(component_metrics$selection_frequency[!active]),
    stringsAsFactors = FALSE
  )
}

recalculate_method_metrics_from_components <- function(method_metrics, component_metrics) {
  if (!is.data.frame(method_metrics) || !nrow(method_metrics) ||
      !is.data.frame(component_metrics) || !nrow(component_metrics)) return(method_metrics)
  metric_columns <- c(
    "active_direction_accuracy", "sign_assignment_accuracy", "active_attribution",
    "null_attribution", "active_selection_frequency", "null_selection_frequency"
  )
  keys <- truth_atomic_key_columns(method_metrics)
  component_indices <- split(
    seq_len(nrow(component_metrics)),
    truth_compound_key(component_metrics, keys)
  )
  method_keys <- truth_compound_key(method_metrics, keys)
  for (i in seq_len(nrow(method_metrics))) {
    if (!isTRUE(method_metrics$fit_success[[i]])) {
      method_metrics[i, metric_columns] <- NA_real_
      next
    }
    matched <- component_indices[[method_keys[[i]]]]
    if (is.null(matched)) next
    summary <- component_metric_summary_base(component_metrics[matched, , drop = FALSE])
    method_metrics[i, metric_columns] <- summary[1L, metric_columns]
  }
  method_metrics
}

correct_sglwqs_direction_truth_support <- function(direction_rows, component_metrics,
                                                    tolerance = truth_support_tolerance()) {
  if (!is.data.frame(direction_rows) || !nrow(direction_rows) ||
      !is.data.frame(component_metrics) || !nrow(component_metrics)) return(direction_rows)
  component_metrics <- component_metrics[component_metrics$method == "SGL-WQS", , drop = FALSE]
  keys <- truth_atomic_key_columns(direction_rows)
  component_keys <- c(keys, "group")
  component_indices <- split(
    seq_len(nrow(component_metrics)),
    truth_compound_key(component_metrics, component_keys)
  )
  direction_keys <- truth_compound_key(direction_rows, component_keys)
  for (i in seq_len(nrow(direction_rows))) {
    matched <- component_indices[[direction_keys[[i]]]]
    if (is.null(matched)) next
    beta <- component_metrics$true_beta[matched]
    if (!length(beta)) next
    active <- if (identical(direction_rows$direction[[i]], "positive")) {
      beta > tolerance
    } else {
      beta < -tolerance
    }
    direction_rows$is_true_null[[i]] <- !any(active)
    direction_rows$active_variable_count[[i]] <- sum(active)
    direction_rows$true_beta_sum[[i]] <- sum(beta[active])
  }
  direction_rows
}

correct_truth_table_support <- function(truth, tolerance = truth_support_tolerance()) {
  if (!is.data.frame(truth) || !nrow(truth)) return(truth)
  truth$IsActive <- abs(truth$True_Beta) > tolerance
  truth$True_Direction <- ifelse(truth$True_Beta > tolerance, "Positive",
    ifelse(truth$True_Beta < -tolerance, "Negative", "None")
  )
  group_keys <- intersect(c(
    "battery", "scenario_id", "family", "data_seed", "split_replicate", "n", "p", "Group"
  ), names(truth))
  group_ids <- truth_compound_key(truth, group_keys)
  for (indices in split(seq_len(nrow(truth)), group_ids)) {
    active_directions <- unique(truth$True_Direction[indices])
    active_directions <- active_directions[active_directions != "None" & !is.na(active_directions)]
    truth$IsActiveGroup[indices] <- any(truth$IsActive[indices])
    truth$Group_Type[indices] <- if (!length(active_directions)) "null" else if (length(active_directions) == 1L) "unidirectional" else "mixed-direction"
  }
  newly_active_tier <- truth$IsActive & (is.na(truth$Effect_Tier) | truth$Effect_Tier == "null")
  truth$Effect_Tier[newly_active_tier] <- "active"
  truth
}
...
write_report_markdown <- function(method_summary,
  ...
  aggregated_attempted <- sum(method_summary$attempted, na.rm = TRUE)
  aggregated_completed <- sum(method_summary$completed, na.rm = TRUE)
  ...
  lines <- c(
    ...
    "- Battery A retains the original local source at `source/sglwqs_old_analysis/sglwqs-main`. Batteries B-H and the imported final C2/D results use the pinned revision source at `source/sglwqs-envint-revision-docs` (Git commit `2fdd519e520a7dad1162810643e175cd616b1154`).",
    "",
    ...
    "",
    paste0("- Core A-E manifest jobs requested: ", attempted),
    paste0("- Core A-E valid completed jobs: ", completed),
    paste0("- Core A-E remaining/incomplete jobs: ", attempted - completed),
    paste0("- All locally aggregated A-H jobs: ", aggregated_completed, "/", aggregated_attempted),
    "- The final exactly paired C2 package is validated separately and is not double-counted in the local A-H total.",
    "",
  ...
  lines <- c(lines, "", "## 8. Original Gaussian Split-Stability Run (Audit/Supporting)", "")
  lines <- c(lines, "The final matched binary-Gaussian comparison is the separately validated C2 package under `results/imported_second_pc/C2_exactly_paired_final`.", "")
  ...
  lines <- c(lines, "- Manuscript-facing summaries should use the exactly paired C2 result set rather than treating the original C run as the final family comparison.")
...
aggregate_and_write_outputs <- function(manifest = NULL, include_smoke = FALSE) {
  ...
  sglwqs_dir <- correct_sglwqs_direction_truth_support(sglwqs_dir, component_metrics)
  if (!is.null(manifest)) {
  ...
  atomic_write_csv(truth_corrections, r2r2_result_file("tables", "truth_support_corrections.csv"))
  ...
  invisible(list(
    ...
    truth_support_corrections = truth_corrections,
