...
qa_generated_data <- function(dat, job) {
  ...
  truth_tolerance <- r2r2_truth_tolerance()
  expected_active <- abs(truth$True_Beta) > truth_tolerance
  expected_direction <- ifelse(truth$True_Beta > truth_tolerance, "Positive",
    ifelse(truth$True_Beta < -truth_tolerance, "Negative", "None")
  )
  assert_or_stop(
    identical(as.logical(truth$IsActive), expected_active),
    "Truth active labels do not match nonzero coefficients."
  )
  assert_or_stop(
    identical(as.character(truth$True_Direction), expected_direction),
    "Truth direction labels do not match coefficient signs."
  )
...
method_metric_row <- function(job,
  ...
  data.frame(
    ...
    failure_stage = if (isTRUE(fit_success)) NA_character_ else "fit",
...
run_method_job <- function(job, settings = r2r2_settings()) {
  ...
  groups <- attr(dat, "groups")
  truth <- attr(dat, "truth")
  split <- make_train_validation_split(nrow(dat), as.integer(job$split_seed), train_prop = settings$train_prop)
  ...
  out <- switch(job$method,
    qgcomp = fit_qgcomp_job(dat, truth, groups, job, settings),
    gWQS = fit_gwqs_job(dat, truth, groups, job, settings, split),
    groupWQS = fit_groupwqs_job(dat, truth, groups, job, settings, split),
    `SGL-WQS` = fit_sglwqs_job(dat, truth, groups, job, settings),
  ...
  result <- tryCatch(
    ...
    error = function(e) {
      ...
      list(
        ...
        method_metrics = method_metric_row(job, FALSE, NA_real_, character(0), error = e),
