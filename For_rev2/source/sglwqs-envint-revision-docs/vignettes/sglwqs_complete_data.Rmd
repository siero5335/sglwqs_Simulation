...
This vignette demonstrates SGL-WQS analysis with large-scale data (n=100,000) having **known true effects**:
...
For large-scale analyses (n > 10,000 and/or n_boot > 200), the checkpoint feature provides:

1. **Crash recovery**: If R crashes or is interrupted, restart and the analysis will resume from the last checkpoint
2. **Memory stability**: Results are saved to disk periodically, reducing memory pressure
3. **Progress monitoring**: Check the checkpoint directory to see completed iterations

```{r checkpoint-usage, eval=FALSE}
...
|-------|-----------|-----------------|
| PCBs | Positive | Conditional validation summary expected to be small (p < 0.05) |
| Metals | Negative | Conditional validation summary expected to be small (p < 0.05) |
| PFASs | Either | Conditional validation summary expected not to be small |

In this simulated example with known effects, the fitted summaries can be
checked against the positive, negative, and null groups used to generate the
data.
