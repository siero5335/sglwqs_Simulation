...
r2r2_result_root <- function() {
  namespace <- Sys.getenv("R2R2_RESULT_NAMESPACE", unset = "")
  if (nzchar(namespace)) {
    namespace <- gsub("[^A-Za-z0-9_.-]+", "_", namespace)
    file.path(r2r2_root(), "results", namespace)
  } else {
    file.path(r2r2_root(), "results")
  }
}

r2r2_result_file <- function(...) {
  file.path(r2r2_result_root(), ...)
}

r2r2_make_dirs <- function() {
  root_dirs <- file.path(r2r2_root(), c("config", "R", "scripts"))
  result_dirs <- file.path(
    r2r2_result_root(),
    c("raw", "summaries", "tables", "figures", "logs", "quarantine")
  )
  invisible(lapply(c(root_dirs, result_dirs), dir.create,
...
make_train_validation_split <- function(n,
                                        split_seed,
                                        train_prop = 0.6,
                                        y = NULL,
                                        family = NULL) {
  ...
  if (identical(family, "binomial") && !is.null(y)) {
    y_levels <- sort(unique(y[!is.na(y)]))
    if (length(y_levels) == 2L) {
      train_idx <- integer(0)
      for (lvl in y_levels) {
        idx_lvl <- which(y == lvl)
        n_train_lvl <- floor(length(idx_lvl) * train_prop)
        if (length(idx_lvl) >= 2L) {
          n_train_lvl <- min(max(n_train_lvl, 1L), length(idx_lvl) - 1L)
        }
        train_idx <- c(train_idx, sample(idx_lvl, size = n_train_lvl, replace = FALSE))
      }
      train_idx <- sort(train_idx)
    } else {
      train_idx <- sample(idx, size = floor(length(idx) * train_prop), replace = FALSE)
    }
  } else {
    train_idx <- sample(idx, size = floor(length(idx) * train_prop), replace = FALSE)
  }
...
write_session_artifacts <- function(prefix = "production") {
  ...
  source_description <- tryCatch(
    read.dcf(file.path(r2r2_sglwqs_source_path(), "DESCRIPTION"), fields = "Version")[[1]],
    error = function(e) NA_character_
  )
  source_main_file <- file.path(r2r2_sglwqs_source_path(), "R", "sglwqs_main.R")
  source_meta <- data.frame(
    item = c(
      "repo_root", "reviewer2_round2_root", "sglwqs_source",
      "sglwqs_source_version", "sglwqs_source_declared_commit",
      "sglwqs_source_main_sha256", "sglwqs_source_git_commit", "workspace_git_commit"
    ),
    value = c(
      r2r2_repo_root(),
      r2r2_root(),
      ...
      source_description,
      Sys.getenv("R2R2_SGLWQS_SOURCE_COMMIT", unset = NA_character_),
      if (file.exists(source_main_file)) digest::digest(file = source_main_file, algo = "sha256") else NA_character_,
