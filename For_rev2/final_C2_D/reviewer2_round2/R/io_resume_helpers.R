make_base_job <- function(battery,
                          ...
                          split_seed = NULL,
                          method_seed = NULL,
                          binomial_target_prevalence = NULL,
  ...
  bootstrap_seed <- ifelse(
    method == "SGL-WQS",
    as.integer(split_seed),
    ifelse(method %in% c("gWQS", "groupWQS"), as.integer(method_seed), NA_integer_)
  )
  if (is.null(binomial_target_prevalence)) {
    binomial_target_prevalence <- if (
      battery %in% c("D_unbalanced_effect_factorial", "E_hard_setting_sample_size") &&
...
make_paired_gaussian_split_stability_jobs <- function() {
  reference_dir <- file.path(
    r2r2_repo_root(), "analysis", "validation_calibration", "output",
    "manifests", "production", "split_stability"
  )
  reference_paths <- sort(list.files(reference_dir, pattern = "\\.rds$", full.names = TRUE))
  assert_or_stop(length(reference_paths) == 400L,
    paste0("Expected 400 completed binary split-stability references; found ", length(reference_paths), ".")
  )
  references <- dplyr::bind_rows(lapply(reference_paths, function(path) {
    x <- readRDS(path)
    assert_or_stop(is.list(x) && is.data.frame(x$job) && nrow(x$job) == 1L,
      paste0("Invalid binary split-stability reference: ", path)
    )
    x$job
  })) |>
    dplyr::arrange(.data$scenario_id, .data$replicate)

  jobs <- purrr::pmap_dfr(
    references[, c(
      "scenario_id", "effect", "n", "within_r", "between_r",
      "data_seed", "replicate", "fit_seed", "job_id"
    )],
    function(scenario_id, effect, n, within_r, between_r,
             data_seed, replicate, fit_seed, job_id) {
      paired <- make_base_job(
        battery = "C2_gaussian_paired_split_stability",
        scenario_id = sprintf("C2_gaussian_paired_%s_n%s", effect, n),
        family = "gaussian",
        n = as.integer(n),
        p = 13L,
        data_seed = as.integer(data_seed),
        method = "SGL-WQS",
        signal_profile = if (identical(effect, "global_null")) "global_null" else "baseline",
        effect_profile = effect,
        group_structure = "legacy13",
        within_rho = as.numeric(within_r),
        cross_rho = as.numeric(between_r),
        split_replicate = as.integer(replicate),
        split_seed = as.integer(fit_seed),
        method_seed = as.integer(fit_seed)
      )
      paired$paired_binary_scenario_id <- scenario_id
      paired$paired_binary_job_id <- job_id
      paired$paired_binary_fit_seed <- as.integer(fit_seed)
      paired
    }
  )
  assert_or_stop(nrow(jobs) == 400L, "Paired Gaussian manifest must contain 400 jobs.")
  jobs
}
