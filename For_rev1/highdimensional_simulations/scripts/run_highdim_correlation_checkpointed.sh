...
echo "[launcher] checkpointed correlation sensitivity run"
echo "[launcher] workers=${GT_N_WORKERS}, boot=${GT_N_BOOT}, seeds=${GT_N_SEEDS}"

Rscript -e 'rmarkdown::render("compare_mixture_methods_highdim_correlation_checkpointed.Rmd", output_file = "compare_mixture_methods_highdim_correlation_checkpointed.html")'
