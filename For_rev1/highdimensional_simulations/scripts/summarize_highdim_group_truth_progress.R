...
cache_dir <- file.path("analysis", "analysis_cache", "highdim_group_truth_checkpointed")
if (!dir.exists(cache_dir)) stop("Group/truth checkpoint cache directory was not found.")

status_files <- list.files(
  cache_dir,
  pattern = "^highdim_group_truth_checkpointed_status_.*\\.csv$",
  full.names = TRUE
)
if (length(status_files) == 0L) stop("No group/truth checkpoint status CSV was found.")
...
out_dir <- file.path("analysis", "results", "highdim_group_truth_progress")
...
cat("Wrote group/truth progress summaries to:", out_dir, "\n")
