...
export HD_P_VALUES=50,100,200,500
export HD_N_SEEDS=30
export HD_N_WORKERS=10
export HD_FORCE_RERUN=false

echo "[launcher] started at $(date)"
echo "[launcher] checkpointed high-dimensional p-sensitivity run"
echo "[launcher] workers=${HD_N_WORKERS}, n=${HD_N_SAMPLE}, boot=${HD_N_BOOT}, seeds=${HD_N_SEEDS}, p=${HD_P_VALUES}"

Rscript -e 'rmarkdown::render("compare_mixture_methods_highdim_p_checkpointed.Rmd", output_file = "compare_mixture_methods_highdim_p_checkpointed.html")'
